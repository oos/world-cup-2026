...
const EXCITEMENT_WORST_RANK = 48;
...
export function getMatchExcitementScore(
  ...
  const harmonicRank = getMatchHarmonicWorldRanking(match);
  if (harmonicRank == null) return null;
  return rankToExcitementScore(harmonicRank);
}
